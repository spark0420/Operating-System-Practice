...
static uint8_t is_stimer_started    = 0;
static uint8_t is_printing_started  = 0;
...
static uint8_t handle_ctrl(uint8_t ch)
{
    switch (ch){
		...
		ch = 0;
    		return ch;
    	case 'b':
    	//restart the timer and print every 2sec
    		if(!is_printing_started) {
			timer_2sec_on();
    			ch = 0;
			is_printing_started = 1;
		} else {
    			timer_2sec_off();
    			ch = 0;
			is_printing_started = 0;
		}
    		return ch;
    	case 'e':
		// Run the prime function
    		checkPrime();
    		ch = 0;
    		return ch;
    }
